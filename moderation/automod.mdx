...
Automod is one command, `/automod`, covering several independent checks. Each has its own enable toggle, so turning one on doesn't affect the others.
...
```javascript Example
/automod word-filter toggle enabled:true action:mute
/automod word-filter add word:slur
```
...
```javascript Example
/automod spam enabled:true max_mentions:5
```
...
```javascript Example
/automod invites allow code:abc123
```
...
```javascript Example
/automod raid enabled:true threshold:6 window_seconds:10 action:kick
```
...
```javascript Example
/automod anti-alt enabled:true min_age_days:7 action:kick
```
...
```javascript Example
/automod link url:https://suspicious-link.com
```
...
```javascript Example
/automod silent-channel add channel:#announcements action:mute
```
