...
`weather` uses Open-Meteo to look up current conditions for a city or country and does not require a key. `summary` shows aggregated messages, reactions, voice time, and the most active channels for the selected period. It does not expose message content.
