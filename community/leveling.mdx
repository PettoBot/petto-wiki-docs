...
```javascript Example
/level enable enabled:true
/level xp min:15 max:25
/level voice-xp amount:5
...
```javascript Example
/level curve difficulty:3 rounding:100
```
...
```javascript Example
/level notify mode:channel channel:#level-ups embed:true every:5
/level notify mode:reply message={user} hit level {level}!
```
...
```javascript Example
/level manage xp action:add user:@Someone amount:500
/level manage xp action:transfer user:@A amount:200 target:@B
```
